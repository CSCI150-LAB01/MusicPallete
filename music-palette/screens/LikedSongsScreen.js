import { FlatList, Pressable, ScrollView, StyleSheet, Text, TextInput, View, Image, ActivityIndicator } from 'react-native'
...
const LikedSongsScreen = () => {
    ...
    const playNextTrack = async() => {
        ...
        if (value.current < savedTracks.length) {
            const nextTrack = savedTracks[value.current]; // play the next song
    ...
    return (
        <>
            <LinearGradient colors={["#350f59", "#190b26"]} style={{ flex: 1 }}>
                <ScrollView style={{ flex: 1, marginTop: 50 }}>
                    ...
                    <Pressable style={{ marginHorizontal: 10, flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginTop: 9 }}>
                        <Pressable style={{ flexDirection: "row", alignItems: "center", gap: 10, backgroundColor: "#42275a", padding: 9, flex: 1, borderRadius: 3, height: 38 }}>
                            <AntDesign name="search1" size={20} color="white" />
                            <TextInput value={input} onChangeText={(text) => handleInputChange(text)} placeholder="Find in Liked Songs" placeholderTextColor={"white"} style={{ fontWeight: "500", color: "white"}} />
                        </Pressable>
                    ...
                    <View style={{ marginHorizontal: 10 }}>
                        ...
                        <Text style={{ color: "white", fontSize: 13, marginTop: 5 }}>50 Songs</Text>
                    </View>

                    <Pressable style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginHorizontal: 10 }}>
                        <Pressable style={{ width: 30, height: 30, borderRadius: 15, backgroundColor: "#140A1F", justifyContent: "center", alignItems: "center" }}>
                            <AntDesign name="arrowdown" size={20} color="white" />
                        </Pressable>

                        <View style={{ flexDirection: "row", alignItems: "center", gap: 10 }}>
                            <MaterialCommunityIcons name="cross-bolnisi" size={24} color="white" />
                            <Pressable onPress={playTrack} style={{ width: 60, height: 60, borderRadius: 35, justifyContent: "center", alignItems: "center", backgroundColor: "#140A1F" }}>
                    ...
                    {searchedTracks.length === 0 ? (
                        <ActivityIndicator size= "large" color= "#cd31c2" />
                    ) : (
                    <FlatList showsVerticalScrollIndicator={false} data={searchedTracks} renderItem={({ item }) => (
            ...
            {currentTrack && (
                <Pressable onPress={() => setModalVisible(!modalVisible)} style={{ backgroundColor: "#140A1F", width: "90%", padding: 10, marginLeft: "auto", marginRight: "auto", marginBottom: 15, position: "absolute", borderRadius: 6, left: 20, bottom: 10, justifyContent: "space-between", flexDirection: "row", alignItems: "center", gap: 10 }}>
                    <View style={{ flexDirection: "row", alignItems: "center", gap: 10 }}>
                        <Image style={{ width: 40, height: 40 }} source={{ uri: currentTrack?.track?.album?.images[0].url }} />
                        <Text numberOfLines={1} style={{ color: "white", fontSize: 13, width: 220, fontWeight: "bold" }}>{currentTrack?.track?.name} •{" "} {currentTrack?.track?.artists[0].name}</Text>
                    </View>
            ...
            <BottomModal visible={modalVisible} onHardwareBackPress={() => setModalVisible(false)}
                ...
                <ModalContent style={{ height: "100%", width: "100%", backgroundColor: "#140A1F" }}>
                    <View style={{ height: "100%", width: "100%", marginTop: 40 }}>
                        <Pressable style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
                            ...
                            <Text style={{ fontSize: 14, fontWeight: "bold", color: "white" }}>{currentTrack?.track?.name}</Text>
